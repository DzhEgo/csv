...
with open('files/grades.csv', mode='r', newline='') as csv_file:
    ...
    for row in csv_reader:
        ...
        average_grade = sum(valid_tests) / len(valid_tests) if valid_tests else 0.0
...
with open('files/new.csv', mode='w', newline='') as csv_file:
    ...
    for (last_name, first_name, ssn), average_grade in data.items():
        csv_writer.writerow({'Last name': last_name, 'First name': first_name, 'SSN': ssn, 'Average Grade': average_grade})
